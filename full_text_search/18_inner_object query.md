# querying inner object 

# delete index
DELETE /movies

# create index with the mapping.
# below mapping for actors is wrong. 
PUT /movies
{
  "mappings": {
    "properties": {
      ...
      "actors.name": { "type" : "text"},
      "actors.birth_date": { "type" : "date"},
      "actors.birth_country": { "type" : "keyword"}
    }
  }
}
...
# 1:1 - search by director birth country
POST /movies/_search
{
  "query": {
    "term": {
...
# 1:N - ignores relationship within the child object
POST /movies/_search
{
  "query": {
    "bool": {
